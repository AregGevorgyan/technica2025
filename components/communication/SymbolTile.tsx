  ...
  return (
    <button
      ...
      className={`
        ...
        bg-[var(--background)] rounded-xl shadow-md
        hover:shadow-lg hover:scale-105
        transition-all duration-200
        border-2 ${highContrast ? 'border-[var(--border)]' : 'border-[var(--border2)]'}
    ...
    >
      ...
      {isGazing && gazeProgress > 0 && (
        <div className="absolute inset-0 rounded-xl overflow-hidden">
          <div
            className="absolute inset-0 bg-[var(--background2)] opacity-30"
            style={{
      ...
      {/* Text */}
      <span className={`font-semibold text-center ${highContrast ? 'text-[var(--text)]' : 'text-[var(--text)]'}`}>
        {tile.text}
      </span>

      {/* Category badge (optional) */}
      {tile.category && (
        <span className="absolute top-1 right-1 text-xs bg-[var(--background)] px-2 py-0.5 rounded-full text-[var(--text)]">
          {tile.category}
        </span>
      )}
    </button>
